...
class AnimalTest
{
    ...
    void isAnimalGoingToWhereItIsSupposedTo()
    {
        ...
        a1.move(MoveDirection.RIGHT); // jest zwrocony w na wschod
        a1.move(MoveDirection.FORWARD);
        Assertions.assertTrue(a1.isAt(new Vector2d(3,2)));
        a1.move(MoveDirection.BACKWARD);
        Assertions.assertTrue(a1.isAt(defaultPosition));

        a1.move(MoveDirection.RIGHT); // jest zwrocony na poludnie
        a1.move(MoveDirection.FORWARD);
        Assertions.assertTrue(a1.isAt(new Vector2d(2,1)));
        a1.move(MoveDirection.BACKWARD);
        Assertions.assertTrue(a1.isAt(defaultPosition));

        a1.move(MoveDirection.RIGHT); // jest zwrocony na zachod
        a1.move(MoveDirection.FORWARD);
    ...
    void animalIsNotGoingOutsideTheMatrix()
    {
        ...
        a3.move(MoveDirection.RIGHT); // jest zwrocony na wschod
        ...
        a5.move(MoveDirection.LEFT);
        a5.move(MoveDirection.FORWARD);
        Assertions.assertEquals(a5.getPosition(),new Vector2d(0,2));

        a6.move(MoveDirection.BACKWARD);
        Assertions.assertEquals(a6.getPosition(),new Vector2d(1,0));

        a7.move(MoveDirection.FORWARD);
        Assertions.assertEquals(a7.getPosition(),new Vector2d(1,4));

        a8.move(MoveDirection.RIGHT);
        a8.move(MoveDirection.FORWARD);
